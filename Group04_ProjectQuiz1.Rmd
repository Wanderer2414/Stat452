...
## ------------------------------------------------------------
## Remove constant hidden features
## ------------------------------------------------------------
...
ridge_lambda <- cv_ridge_relu$lambda.min
...
lasso_lambda <- cv_lasso_relu$lambda.min
...
enet_cv <- vector("list", length(alphas))
cv_error <- numeric(length(alphas))
...
best_lambda <- enet_cv[[best]]$lambda.min
...
## ------------------------------------------------------------
## Number of active hidden features
## ------------------------------------------------------------

ridge_active <-
  sum(as.vector(coef(ridge_relu))[-1] != 0)

lasso_active <-
  sum(as.vector(coef(lasso_relu))[-1] != 0)

enet_active <-
  sum(as.vector(coef(enet_relu))[-1] != 0)
...
relu_summary <- data.frame(
  ...
  CV_Error = c(
    min(cv_ridge_relu$cvm),
    min(cv_lasso_relu$cvm),
    min(cv_error)
  ),

  Active_Hidden_Features = c(
    ridge_active,
    lasso_active,
    enet_active
  )
...
knitr::kable(
  relu_summary,
  digits = 4,
  caption = "Performance of regularized models using fixed ReLU features."
)


```

The hidden-layer parameters were generated only once using the declared random-feature seed (**240404**) and remained fixed throughout the experiment. Therefore, the random weight vectors \(a_m\) and biases \(c_m\) were **not learned**. Instead, ridge, lasso, and elastic net estimated only the output-layer coefficients associated with the generated hidden features.

Constant hidden features were identified using **only the training data** and removed before model fitting. The same subset of retained hidden features was then applied to the test data, preventing information leakage from the holdout set.

Table X reports the selected tuning parameters, the minimum five-fold cross-validation error, and the number of active hidden features. Ridge generally retains almost all hidden features because the \(L_2\) penalty continuously shrinks coefficients without producing exact zeros. Lasso produces a sparse solution by setting many output weights exactly to zero, while elastic net exhibits behaviour between ridge and lasso by balancing shrinkage and sparsity.
