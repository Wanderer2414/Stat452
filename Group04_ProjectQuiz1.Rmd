...
retained: `lcavol`, `lweight`, `age`, `lbph`, `svi`, `lcp`, `gleason`, and
`pgg45`. Every one is numeric, so the template's type guard passes and the design
matrix needs no factor encoding. The overview table below records what each
variable means, the unit it is measured in, and the integrity checks it survived.
