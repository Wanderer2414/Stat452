...
relu_summary <- data.frame(
  ...
  Lambda_min = c(
    ...
    enet_relu_cv[[best_relu_enet_idx]]$lambda.min
  ),
  ...
  Lambda_1se = c(
    ...
    enet_relu_cv[[best_relu_enet_idx]]$lambda.1se
  ),
...
pred_ridge <- as.numeric(predict(ridge, newx = x_test, s = "lambda.min"))
pred_lasso <- as.numeric(predict(lasso_cv, newx = x_test, s = "lambda.min"))

best_orig_enet_model <- elastic_fits[[paste0("alpha_", best_orig_enet_alpha)]]
pred_enet <- as.numeric(predict(best_orig_enet_model, newx = x_test, s = "lambda.min"))

pred_relu_ridge <- as.numeric(predict(ridge_relu_cv, newx = H_test, s = "lambda.min"))
pred_relu_lasso <- as.numeric(predict(lasso_relu_cv, newx = H_test, s = "lambda.min"))

best_relu_enet_model <- enet_relu_cv[[best_relu_enet_idx]]
